...
def user_register():
    ...
    if username and password and telephone:  # 注意if条件中 "" 也是空, 按False处理
        sql1 = "SELECT username FROM user WHERE username = '{}'".format(
            username)
        res1 = db.select_db(sql1)
        print("查询到用户名 ==>> {}".format(res1))
        sql2 = "SELECT telephone FROM user WHERE telephone = '{}'".format(
            telephone)
...
def user_login():
    ...
    if username and password:  # 注意if条件中空串 "" 也是空, 按False处理
        ...
        if res2:
            ...
            return jsonify({"code": 0, "login_info": login_info, "msg": "登录成功！"})
...
def user_update(id):  # id为准备修改的用户ID
    ...
    if admin_user and token and new_password and new_telephone:  # 注意if条件中空串 "" 也是空, 按False处理
        if not (new_sex == "0" or new_sex == "1"):
            ...
            redis_token = redis_db.handle_redis_token(
                admin_user)  # 从redis中取token
            if redis_token:
                if redis_token == token:  # 如果从redis中取到的token不为空，且等于请求body中的token
                    sql1 = f"SELECT role FROM user WHERE username = '{admin_user}'"
                    res1 = db.select_db(sql1)
                    ...
                    if user_role == 0:  # 如果当前登录用户是管理员用户
                        ...
                        sql3 = "SELECT telephone FROM user WHERE telephone = '{}'".format(
                            new_telephone)
                        res3 = db.select_db(sql3)
                        print("返回结果：{}".format(res3))
                        print("查询到手机号 ==>> {}".format(res3))
                        if not res2:  # 如果要修改的用户不存在于数据库中，res2为空
                            return jsonify({"code": 4005, "msg": "修改的用户ID不存在，无法进行修改"})
                        elif res3:  # 如果要修改的手机号已经存在于数据库中，res3非空
                            return jsonify({"code": 4006, "msg": "手机号已被注册，无法进行修改"})
                        else:
                            # 如果请求参数不传address，那么address字段不会被修改，仍为原值
                            if not new_address:
                                new_address = res2[0]["address"]
                            # 把传入的明文密码通过MD5加密变为密文
                            new_password = get_md5(
                                res2[0]["username"], new_password)
                            sql3 = "UPDATE user SET password = '{}', sex = '{}', telephone = '{}', address = '{}' " \
                                   "WHERE id = {}".format(
                                       new_password, new_sex, new_telephone, new_address, id)
                            db.execute_db(sql3)
                            print("修改用户信息SQL ==>> {}".format(sql3))
                            return jsonify({"code": 0, "msg": "恭喜，修改用户信息成功！"})
                    else:
                        return jsonify({"code": 4004, "msg": "当前用户不是管理员用户，无法进行操作."})
                else:
                    return jsonify({"code": 4003, "msg": "token口令不正确"})
            else:
                return jsonify({"code": 4002, "msg": "当前用户未登录，请检查！！！"})
    else:
        return jsonify({"code": 4001, "msg": "管理员用户/token口令/密码/手机号不能为空"})
...
def user_delete(username):
    ...
    if admin_user and token:
        ...
        if redis_token:
            if redis_token == token:  # 如果从redis中取到的token不为空，且等于请求body中的token
                sql1 = "SELECT role FROM user WHERE username = '{}'".format(
                    admin_user)
                res1 = db.select_db(sql1)
                print("根据用户名 【 {} 】 查询到用户类型 == >> {}".format(admin_user, res1))
                user_role = res1[0]["role"]
                if user_role == 0:  # 如果当前登录用户是管理员用户
                    sql2 = "SELECT * FROM user WHERE username = '{}'".format(
                        username)
                    res2 = db.select_db(sql2)
                    print(sql2)
                    ...
                    if not res2:  # 如果要删除的用户不存在于数据库中，res2为空
                        ...
                        sql3 = "DELETE FROM user WHERE username = '{}'".format(
                            username)
                        db.execute_db(sql3)
                        print("删除用户信息SQL ==>> {}".format(sql3))
                        return jsonify({"code": 0, "msg": "恭喜，删除用户信息成功！"})
                else:
                    return jsonify({"code": 3004, "msg": "当前用户不是管理员用户，无法进行操作."})
            else:
                return jsonify({"code": 3003, "msg": "token口令不正确"})
        else:
            return jsonify({"code": 3002, "msg": "当前用户未登录"})
    else:
        return jsonify({"code": 3001, "msg": "管理员用户/token口令不能为空"})
